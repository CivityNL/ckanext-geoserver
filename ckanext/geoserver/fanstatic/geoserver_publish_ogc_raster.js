'use strict';
ckan.module('geoserver_publish_ogc_raster', function($, _) {
	return {
		initialize: function() {
			var form, res, obj;
			obj = this;
		...
		_onClick: function(e) {
			...
			obj.getExtras(obj.options.package, function(res) {
				...
				if (obj.options.resource) {
					if (obj.published) {
						obj.sandbox.client.getTemplate('geoserver_unpublish_ogc_form_raster.html', obj.options, obj._onReceiveUnpublishSnippetSingle);
						return true;
					} else {
						obj.sandbox.client.getTemplate('geoserver_publish_ogc_form_raster_single.html', obj.options, obj._onReceivePublishSnippetSingle);
						return true;
					}
				} else {
					if (obj.published) {
						obj.sandbox.client.getTemplate('geoserver_unpublish_ogc_form_raster.html', obj.options, obj._onReceiveUnpublishSnippetMulti);
						return true;
					} else {
						obj.sandbox.client.getTemplate('geoserver_publish_ogc_form_raster_multi.html', obj.options, obj._onReceivePublishSnippetMulti);
						return true;
					}
				}
			});
		...
		_onReceivePublishSnippetMulti: function(html) {
			...
			//append new modal into body
			$('body').append(html);
			resourceInput = $('body').find('#resource_id').val("raster_multi");
			packageInput = $('body').find('#package_id').val(obj.options.package);
			//show modal
			$('#publish_ogc_modal').modal('show');
			$("#publish_ogc_modal").on('shown', function() {
